...
test.describe('Equipment Browsing', () => {
  ...
  test('should show equipment count or empty state', async ({ authenticatedPage }) => {
    ...
    const hasEquipment = await authenticatedPage.getByTestId('equipment-grid').isVisible().catch(() => false);
    const isEmpty = await authenticatedPage.getByTestId('equipment-grid-empty').isVisible().catch(() => false);
  ...
  test('should open equipment details on click', async ({ authenticatedPage }) => {
    ...
    if (equipmentId) {
      ...
      // Wait for the details sheet to open
      await authenticatedPage.waitForTimeout(500);
      
      // Check if equipment details sheet opened
      const detailsSheet = authenticatedPage.getByTestId('equipment-details-sheet');
      const isOpen = await detailsSheet.isVisible().catch(() => false);
  ...
  test('should allow adding equipment to cart', async ({ authenticatedPage }) => {
    ...
    const availableEquipment = authenticatedPage.locator('[data-testid^="equipment-add-to-cart-"]').first();
    ...
    const initialCount = initialCountVisible ? parseInt((await cartCount.textContent()) || '0') : 0;
    ...
    const newCountVisible = await cartCount.isVisible();
    expect(newCountVisible).toBeTruthy();
  ...
  test('should navigate to cart when clicking cart indicator', async ({ authenticatedPage }) => {
    ...
    // URL should contain 'cart' or 'checkout' or 'reservation'
