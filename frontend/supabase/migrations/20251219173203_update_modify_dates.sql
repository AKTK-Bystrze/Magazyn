...
BEGIN
    ...
    IF v_credit_adjustment != 0 THEN
        ...
        VALUES (
            ...
            CASE 
                WHEN v_credit_adjustment > 0 THEN FORMAT('Refund for shortening reservation (%s days to %s days)', v_old_days, v_new_days)
                ELSE FORMAT('Charge for extending reservation (%s days to %s days)', v_old_days, v_new_days)
            END,
            p_reservation_id,
    ...
    UPDATE reservations SET start_date = p_new_start_date, end_date = p_new_end_date, updated_at = NOW()
    WHERE id = p_reservation_id RETURNING * INTO v_reservation;

    RETURN jsonb_build_object(
        'id', v_reservation.id, 'start_date', v_reservation.start_date, 'end_date', v_reservation.end_date,
        'status', v_reservation.status, 'updated_at', v_reservation.updated_at, 'old_cost', v_old_cost,
        'new_cost', v_new_cost, 'credit_adjustment', v_credit_adjustment, 'new_balance', v_new_balance
    );
END;
$$;
