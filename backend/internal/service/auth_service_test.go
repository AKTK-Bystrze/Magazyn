...
func TestAuthService_GetSession(t *testing.T) {
	t.Run("successful session retrieval", func(t *testing.T) {
		...
		mockDB.On("WithUserToken", userToken).Return(mockDB)
		...
		session, err := s.GetSession(context.Background(), userId, userToken)
	...
	t.Run("profile not found", func(t *testing.T) {
		...
		mockDB.On("From", "profiles").Return(mockQuery)
		mockQuery.On("Select", "*", "exact", false).Return(mockFilter)
		mockFilter.On("Eq", "id", "user-123").Return(mockFilter)
		...
		session, err := s.GetSession(context.Background(), "user-123", userToken)
