...
BEGIN
    ...
    INSERT INTO credit_history (user_id, amount, reason, description, author_id) VALUES (p_user_id, -p_total_cost, 'reservation_charge', 'For equipment reservation', p_user_id);
    FOR v_res_item IN SELECT * FROM jsonb_array_elements(p_reservations) LOOP
        ...
        SELECT COUNT(*) INTO v_conflict_count FROM reservations WHERE equipment_id = v_equipment_id AND status IN ('PENDING', 'RENTED') AND start_date <= v_end_date AND end_date >= v_start_date;
        IF v_conflict_count > 0 THEN RAISE EXCEPTION 'Conflict detected'; END IF;
        INSERT INTO reservations (user_id, equipment_id, start_date, end_date, status) VALUES (p_user_id, v_equipment_id, v_start_date, v_end_date, 'PENDING') RETURNING id INTO v_reservation_id;
