...
BEGIN
    ...
    v_user_id := v_reservation.user_id; v_equipment_id := v_reservation.equipment_id; v_old_start_date := v_reservation.start_date; v_old_end_date := v_reservation.end_date;
    ...
    v_old_days := (v_old_end_date - v_old_start_date) + 1; v_new_days := (p_new_end_date - p_new_start_date) + 1;
    v_old_cost := v_old_days * v_credit_per_day; v_new_cost := v_new_days * v_credit_per_day; v_credit_adjustment := v_old_cost - v_new_cost;
    ...
    IF v_credit_adjustment != 0 THEN INSERT INTO credit_history (user_id, amount, reason, description, reservation_id, author_id) VALUES (v_user_id, v_credit_adjustment, 'reservation_adjustment', 'Adjustment', p_reservation_id, p_changed_by_user_id); END IF;
